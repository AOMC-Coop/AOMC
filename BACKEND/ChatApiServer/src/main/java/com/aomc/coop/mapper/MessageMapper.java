...
public interface MessageMapper {
    ...
    //채널의 메세지조회
    @Select("SELECT m.message_idx, m.content, m.channel_idx, u.nickname, u.idx as user_idx, u.image, DATE_FORMAT(send_date, '%W, %M %D') as send_date, DATE_FORMAT(send_date, '%l:%i %p') as send_time FROM messages m, users u WHERE m.user_idx=u.idx AND channel_idx = #{channelIdx} ORDER BY m.message_idx desc LIMIT #{start}, 10")
